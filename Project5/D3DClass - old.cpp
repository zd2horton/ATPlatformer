	...
	HWND hwnd, bool fullscreen, float s_depth, float s_near)
{
	...
	/*error = wcstombs_s(&string_length, m_video_card_description, 128, 
	...
	screen_aspect = (float)s_width / (float)s_height;
	m_projection_matrix = XMMatrixPerspectiveFovLH(fov, screen_aspect, s_near, s_depth);
...
void D3DClass::beginScene(float red, float green, float blue, float alpha)
{
	...
	m_device_context->ClearDepthStencilView(m_depth_stencil_view, D3D11_CLEAR_DEPTH,
		1.0f, 0);
	return;
}
